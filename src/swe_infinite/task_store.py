...
def build_task_instance(
    ...
    # (common when the problem statement was generated by LLM).
    issue_title = extracted.get("issue_title", "")
    issue_body = extracted.get("issue_body", "")
    if issue_title and issue_body.lstrip().startswith(issue_title):
        # Body already contains the title — use body as-is
        problem_statement = issue_body.strip()
    elif issue_title and issue_body:
        problem_statement = f"{issue_title}\n\n{issue_body}".strip()
    else:
        problem_statement = (issue_title or issue_body).strip()
